...
export function buildSavePath(attachmentId, filename) {
...
function truncate(text) {
    if (text.length <= TEXT_TRUNCATION_LIMIT)
        return { text };
    return { text: text.slice(0, TEXT_TRUNCATION_LIMIT), textTruncated: true };
}
function isTextLikeMime(contentType, ext) {
...
function isImageMime(contentType, ext) {
    if (contentType.toLowerCase().startsWith('image/'))
        return true;
    return ['.png', '.jpg', '.jpeg', '.gif', '.webp', '.tiff', '.heic'].includes(ext);
}
async function extractPdf(savedPath) {
    try {
        ...
        return {
            text: null,
            extractionMethod: 'pdf-parse',
            extractionError: `PDF parse failed: ${err.message}`,
        };
    }
}
async function extractDocx(savedPath) {
    try {
        ...
        return {
            text: null,
            extractionMethod: 'mammoth',
            extractionError: `DOCX parse failed: ${err.message}`,
        };
    }
}
async function extractXlsx(savedPath) {
    try {
        ...
        return {
            text: null,
            extractionMethod: 'xlsx',
            extractionError: `XLSX parse failed: ${err.message}`,
        };
    }
}
async function extractPptx(savedPath) {
    try {
        ...
        return {
            text: null,
            extractionMethod: 'officeparser',
            extractionError: `PPTX parse failed: ${err.message}`,
        };
    }
}
async function readTextPlain(savedPath) {
    try {
        ...
        return {
            text: null,
            extractionMethod: 'utf8',
            extractionError: `Text read failed: ${err.message}`,
        };
    }
}
async function extractImageOcr(savedPath) {
    try {
        ...
        return {
            ...
            extractionError: 'OCR binary unavailable — run scripts/build-ocr.sh (requires swiftc on macOS).',
        };
    }
    try {
        ...
        return {
            text: null,
            extractionMethod: 'ocr-vision',
            extractionError: `OCR failed: ${err.message}`,
        };
    }
}
export async function extractText(savedPath, contentType) {
...
export async function saveAndExtract(bytes, metadata) {
